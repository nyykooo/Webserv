...
void setup(const char *file, std::vector<Configuration> &confserv)
{
	...
	{
		throw Configuration::WrongConfigFileException("Error opening file: " + std::string(file) + " " + std::string(strerror(errno)));
	}
	while (std::getline(configFile, line))
	{
		...
		if (line.find('{') == std::string::npos || line.find('}') != std::string::npos || (line[line.size() - 1] != '{' 
			...
			throw Configuration::WrongConfigFileException("invalid syntax.");
		line.erase(line.size() - 1);
		if (word == "server") {
