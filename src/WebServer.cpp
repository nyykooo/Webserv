...
/*   Updated: 2025/07/14 21:20:33 by brunhenr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
WebServer::WebServer(const std::vector<Configuration>& configs) : _configurations(configs)
{
	initEpoll();
	_events = new struct epoll_event[MAX_EVENTS];

	for (std::vector<Configuration>::const_iterator config_it = _configurations.begin(); config_it != _configurations.end(); ++config_it)
	{
		// std::set para hosts
		const std::set<std::pair<std::string, std::string> >& hosts = config_it->getHost(); // aqui está o grande ponto a ser analisado
		// Iterador para std::set
		for (std::set<std::pair<std::string, std::string> >::const_iterator host_it = hosts.begin(); host_it != hosts.end(); ++host_it)
		{
			std::string ip = host_it->first;
			std::string port = host_it->second;
			std::string key = ip + ":" + port;
			
			// Verificação se já existe um servidor para este ip:port
			...
			if (!server_exists)
			{
				...
				if (epoll_ctl(_epoll_fd, EPOLL_CTL_ADD, server->getSocketFd(), &server->getEvent()) == -1) {
					throw std::runtime_error("epoll_ctl failed for " + key);
				}
			}
		}
	}
